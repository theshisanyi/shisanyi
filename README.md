...
> **注意**：Steam 爬取需要直连 `store.steampowered.com`；游侠/逗游/3DM 通过 Bing 搜索引擎间接查找。部分公司网络可能拦截外网。
...
| 来源 | 获取内容 | 说明 |
|------|----------|------|
| Steam | 封面图、官方简介、标签、发售日、购买链接 | 需直连 Steam API |
| 豆瓣 | 中文标题、评分 | 直接搜索 |
| 游侠 | 简介、标签、封面图 | 通过 Bing 搜索 |
| 逗游 | 标题 | 通过 Bing 搜索 |
| 3DM | 标题 | 通过 Bing 搜索 |

各来源独立运行，单个失败不影响其他。Steam 网络不通时，可直接通过管理后台手动添加游戏。
